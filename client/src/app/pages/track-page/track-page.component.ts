...
export class TrackPageComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.spotifyService.getTrack(this.trackId).then(data => {
      ...
      console.log('Calling getTrack function in spotifyService');
      console.log(this.track);
    });

    //Get the audio features for track
    this.spotifyService.getAudioFeaturesForTrack(this.trackId).then(data => {
      this.audioFeatures = data;
      //For debugging purposes
      console.log('Calling getAudioFeaturesForTrack function in spotifyService');
